...
bitset<msize*msize> vmap;
void dbgset(vector<IntPair>& cset)
{
...
int main()
{
    ...
    for( int i = 3; i<=target; ++i){
        for(auto iter = vset[i].begin(); iter != vset[i].end(); ++iter){
            if(!vmap.test(iter->first*msize+iter->second)){
                vmap.set(iter->first*msize+iter->second);
                ++cnt;
            }
        }
    }
