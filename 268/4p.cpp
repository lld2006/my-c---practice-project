...
#include <algorithm>
#include <numeric>
//lesson, this is a stupid way of doing it. the correct way should consider how the set may
//evole when we use inclusion-exclusion priniciple!!!! use combinatorial factors!
...
i64 inc_ex_search(int pos, i64 nupper, i64 product, vector<int>& ndiv )
{
    ...
    for(unsigned int i = pos; i< ndiv.size(); ++i){
        if(!ndiv[i]) continue;
        i64 nx = product * primes[i];
        sum += nupper/nx;
        i64 tsum = inc_ex_search(i+1, nupper, nx, ndiv);
        sum -= tsum;
    }
    return sum;
}
