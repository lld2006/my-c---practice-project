...
bool isPrimeJoint(int p1, int p2, vector<int>& primes){
    ...
    if(px > limit)
        printf("%d %d %d\n", p1, p2, px);
    assert(px < limit );
    return binary_search(primes.begin(),primes.end(), px);
}

int checkInsert(int shift, vector<int>& pvec, vector<int>& primes){
    int prime2 = primes[shift];
    for(unsigned int i = 0; i<pvec.size(); ++i){
        int prime = pvec[i];
        if( !isPrimeJoint(prime, prime2, primes) || !isPrimeJoint(prime2, prime,primes)){
            return 0;
        }
    }
    pvec.push_back(prime2);               
    if(pvec.size()==5)
        return 2;
    else
        return 1;
