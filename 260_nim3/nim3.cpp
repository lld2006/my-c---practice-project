...
//lesson first index3o is not correct!
//lesson second process is not quite right especially xy
//need to speed up the efficiency
...
void process_x(int x, int y, int z)
{
    for(int i = x+1; i <= y; ++i)
        ...
        //fbit.set(index3o(i,y,z));
    for(int i = y+1; i <= z; ++i)
        set_fbit(y,i,z, x,y, z);
        //fbit.set(index3o(y,i,z));
    for(int i = z+1; i <= limit-x; ++i)
        set_fbit(y,z,i, x,y,z);
        //fbit.set(index3o(y,z,i));
}
...
void process_xz(int x, int y, int z)
{
    if(y-x < limit -z){
        for( int t = 1; t<= y-x; ++t)
            ...
            //fbit.set(index3o(x+t,y,z+t));
        for( int t =y-x+1;t<=limit-z;++t)
            set_fbit(y,x+t,z+t,x,y,z);
            //fbit.set(index3o(y,x+t,z+t));
    }else{
...
int main()
{
    ...
    //    for(unsigned int j = i+1; j< va.size();++j){
    //        int x1 = (va[j]-va[i]);
    //        int y1 = (vb[j]-vb[i]);
    //        int z1 = (vc[j]-vc[i]);
    //        if(x1 == 0 && y1 ==0 && z1 !=0) assert(0);
    //        if(x1 != 0 && y1 ==0 && z1 ==0) assert(0);
    //        if(x1 == 0 && y1 !=0 && z1 ==0) assert(0);
    //        if(x1 ==0 && y1==z1) assert(0);
    //        if(y1 ==0 && x1==z1) assert(0);
    //        if(z1 ==0 && x1==y1) assert(0);
    //        if((x1==y1) && (y1==z1))assert(0);
    //        
    //    }
    //}
    printf("%lld\n", total);
